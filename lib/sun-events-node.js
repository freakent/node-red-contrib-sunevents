...
module.exports = function(RED) {
	...
	function SunEventsNode(config) {
		...
		node.on('input', function(msg, send) {
			...
			node.context().set("msg", Object.assign({}, msg))
			...
			node.log(`calculating sunevents for lat ${lat} and lng ${lng}`)
			node.sunevents.refresh(lat, lng)			
			node.update_status()
			//msg.payload.sunevents = node.sunevents._missed_events
			let payload = {...msg, payload: { ...msg.payload, sunevents: node.sunevents._missed_events } }
			send([null, payload ])
		})
