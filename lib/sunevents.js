...
class SunEvents extends EventEmitter {
    constructor(lat, lng, modes) {
        super();
        ...
        this.debugMode = modes.debug || false;
        this.testMode = modes.test || false;
    ...
    start() {
        ...
        this.setTimers(new Date());

        this.debug("Start job to calculate daily sunevents for tomorrow onwards");
    ...
    setTimers(date) {
        ...
        this.debug("Calculating times for %s", endOfDay)
        
        let times = SunCalc.getTimes(endOfDay, this.lat, this.lng)                
        ...
        for (let [event, datetime] of Object.entries(times)
            ...
            let hrs = datetime.diff(now, "hours")

            if(datetime.isAfter(now)) {
                this.debug("Emitting %s %s in %d hours", event, datetime.calendar(), hrs);
                ...
                this.debug("Ignoring %s, was %s, %d hours ago", event, datetime.calendar(), hrs);
            }
        }
    }
