...
class SunEvents extends EventEmitter {
    constructor(lat, lng, modes, timeshift) {
        super();
    ...
    setTimers(date) {
        ...
        for (let [event, datetime] of Object.entries(times)
            ...
            // calculating from the timeshift the new time to fire the evnet
            if(this.timeshift.substring(0, 1) === "-") {
                datetime.subtract(this.timeshift.substring(1,this.timeshift.length), 'minutes');
            } else {
                datetime.add(this.timeshift.substring(1,this.timeshift.length), 'minutes');
            }
